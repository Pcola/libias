...
/**
 * Hmac signer Created by Michael DESIGAUD on 15/02/2016.
 */
public class HmacSigner {  
  ...
  /**
   * Get a signed JWT The issuer (user id) and the custom properties are stored in the JWT to be retrieve later
   *
  ...
  public static String generateSecret() throws HmacException {
    try {
      ...
      LOG.error(Error.GENERATE_SECRET, e);
      throw new HmacException(Error.GENERATE_SECRET.getDescription(), e);
    }
  }
  ...
  private static String generateJWT(String secret, String jwtID, String iss, Integer ttl, Map<String, String> claims) throws HmacException {
    try {
      ...
      LOG.error(Error.GENERATE_JWT, e);
      throw new HmacException(Error.GENERATE_JWT.getDescription(), e);
    }
  }
