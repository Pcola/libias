...
public class StatisticService {
  ...
  public List<SiteStatistics> getSiteStatistics(SourceSystem sourceSystem) {
    SourceSystem system = (sourceSystem != null) ? sourceSystem : SourceSystem.COGNITEC;

    // count all incidents with assigned workplace
    Query q;
    if (system == SourceSystem.GES) {
      q = entityManager.createQuery("select i.workplace.id, i.workplace.workplace, i.priority.priorityId, i.status.statusId, count(i.status.statusId) "
